 ...
 */
function getChat() {
    let getChatUrl = $('#get-chat-url').val();
    let data = {
        'roomId': $('#room-id').val(),
        'offset': $('#offset').val(),
        ts: new Date().getTime()
    };
    $.getJSON(getChatUrl, data, function (response) {
        $('#offset').val(response.chatInfo.offset);
        // チャットメッセージ生成
        for (const message of response.chatInfo.messages) {
            let outputLine = message.posted_datetime;
            let systemMsgClass = "";
 ...
 */
function submitChat() {
    let url = $('#post-chat-url').val();
    let roomId = $('#room-id').val();
    let message = $('#chat-message').val();
    let csrfToken = $('#chat-info input[name="_csrfToken"]').val();
